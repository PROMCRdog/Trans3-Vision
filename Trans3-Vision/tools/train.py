...
wandb.init(
    ...
    project="Trans3-Vision1",
    
    # track hyperparameters and run metadata
    config={
    "learning_rate": 0.0001,
    "architecture": "VIT",
    "dataset": "Grounded-ID2",
    "epochs": 200,
    }
)
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                logging.info(
                    ...
                    "Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        ...
                        eta_string))
                wandb.log({
                    "Epoch":epoch, "Iters":iteration % iters_per_epoch, "Lr":self.optimizer.param_groups[0]['lr'], 
                    "Loss":losses_reduced.item(), "Cost Time":str(datetime.timedelta(seconds=int(time.time() - start_time))), 
                    "Estimatd Time":eta_string
                })
